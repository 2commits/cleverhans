...
def build_agent(fixture: dict[str, Any], vector: dict[str, Any]) -> tuple[cleverhans_agent.Agent, list[dict[str, Any]]]:
    ...
    for action in fixture["registry"]["actions"]:
        ...
        def handler(params, principal, *, _id=action_id, _script=script["handler"]):
            ...
            if "fail" in _script:
                raise cleverhans_agent.Rejected(_script["fail"])
        ...
        if "dry_run" in script and script["dry_run"] is not None:
            ...
            def dry_run(params, principal, *, _script=script["dry_run"], _calls=calls):
                ...
                _calls["n"] += 1
                if "fail" in behavior:
                    raise cleverhans_agent.Rejected(behavior["fail"])
