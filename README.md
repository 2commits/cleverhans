...
| `crates/cleverhans-py` | PyO3 extension module (packaged from `python/cleverhans`) |
...
| `python/cleverhans` | Python backend binding (maturin package `cleverhans`) |
| `python/examples` | Example Python host (scripted smoke flow) |
...
python/cleverhans/.venv/bin/python python/examples/demo.py
...
# Python binding (once): cd python/cleverhans && python3.12 -m venv .venv \
#   && .venv/bin/pip install maturin pytest
cd python/cleverhans && .venv/bin/maturin develop && .venv/bin/python -m pytest tests/
```
